...
const makeSeries = (resAmount) =>
    Array.from(
        { length: 90 },
        (_, i) => (0.8 * (25 + 2 * i * resAmount)) / (125 + 2 * i * resAmount),
    );
...
const datasets = [9, 8, 7, 6, 5, 4, 3, 2, 1, 0].map((resAmount) => ({
    label: `${resAmount} Res`,
...
const htmlLegendPlugin = {
    ...
    beforeInit(chart) {
        const legendContainer =
            chart.options.plugins.htmlLegendPlugin.legendDiv;
