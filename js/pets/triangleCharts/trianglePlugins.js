...
import {
	...
	clearTrident,
	drawTrident,
	SquareToTriangleCoor,
	DOMToSquareCoor,
	initializeTickDOM,
	initializeLabelDOM,
	createLabelImage,
	getPixel,
	buildTriangleDataset,
	getX,
	getY,
	...
	beforeDraw(chart) {
		...
		cardinals.forEach((cardinal) => {
			plugin[cardinal].ticks.forEach((tick) => {
				var coor
				if (cardinal === 'left') coor = getPixel(chart.scales, [tick.percent, 0])
		...
		if (data.left >= 0 && data.right >= 0 && data.bottom >= 0) {
			...
			cardinals.forEach((cardinal) => {
				function isWithin(toTest, target, radius) {
					...
					return toTest >= min && toTest <= max ? 0 : 1
				}
				plugin[cardinal].ticks.forEach(
					(tick) =>
...
export const lineOnClickPluginFactory = (pluginConfig) => ({
	...
	afterEvent(chart, args) {
		...
		this.selectedPoint = element ? [element.element.x, element.element.y] : undefined
...
export const tooltipPluginFactory = (pluginConfig) => ({
	beforeInit(chart) {
		...
		const externalTooltipHandler = (context) => {
			...
			plugin.statTexts.forEach((text, i) => (text.textContent = ' ' + attributes[i]))
