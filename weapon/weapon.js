...
function createWikipediaContainer(weapon) {
	...
	const renderStars = (v) => [...Array(5)].map((_, i) => (v > i ? '&starf;' : '&star;')).join('')

	const makeStarDisplay = (statistic) =>
		make('div', {
			innerHTML: '<span>' + statistic.name + '</span>' + renderStars(statistic.stars),
		})

	const makeAliasString = (aliases) => (aliases[0] ? 'aliases: ' + weapon.aliases.join(', ') : '')
...
function createWikipediaTable(weapon) {
	...
	const displayStats = [...weapon.statConfig, ...buffStats, weapon.wpStatConfig].filter(Boolean)

	displayStats.forEach((stat) =>
		table.append(
			make(
				'div',
				{ className: 'wikipedia-stat-row' },
				[stat.min, stat.max].map((extreme) =>
					make('div', { textContent: extreme + stat.unit }, stat.emoji.map(makeImg))
				)
			)
		)
	)
